...
class CircleWithImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> widgets = [];
    List<Widget> smallCircles = List<dynamic>(10).map((dynamic _) => _smallCircle(MediaQuery.of(context).size)).toList();
    List<Widget> largeCircles = List<dynamic>(5).map((dynamic _) => _largeCircle(MediaQuery.of(context).size)).toList();
